import { useState } from "react";
import ProgressBarReceiver from "./progressBarReceiver";

const PostReceiver = () => {
    const initialCaption = "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Assumenda libero numquam ut inventore, voluptatem nobis nostrum officiis vero nesciunt at. Eius doloremque fugit dignissimos tempora perferendis magnam sapiente cupiditate accusantium.";
    const maxLength = 100; // Maximum characters to show initially

    const [caption, setCaption] = useState(initialCaption.slice(0, maxLength));
    const [showFullCaption, setShowFullCaption] = useState(false);

    const toggleCaption = () => {
        setShowFullCaption(!showFullCaption);
        setCaption(showFullCaption ? initialCaption.slice(0, maxLength) : initialCaption);
    };

    return (
        <div className=" rounded mt-10 w-full  gap-2 bg-white">
            <div className="bg-slate-200">
                <div className="flex flex-row">
                    {/* <img src="" alt="" /> */}
                    <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" className="bi bi-person-circle" viewBox="0 0 16 16">
                        <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0" />
                        <path fillRule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1" />
                    </svg>
                    <p className="ml-4">Receiver</p>
                </div>
                <div className="mt-3 ml-3">
                    <p>
                        Caption: {caption}
                        {/* {!showFullCaption && <span onClick={toggleCaption} className="cursor-pointer text-blue-500">...Show More</span>}
                        {showFullCaption && <span onClick={toggleCaption} className="cursor-pointer text-blue-500">...Show Less</span>} */}
                    </p>
                </div>
                {/* <div className="w-full bg-cover">
                    <img className="w-full" src="data:image/png;base64," alt="" />
                </div> */}

                <div className="flex flex-col items-center">
                    <ProgressBarReceiver />
                    {/* <button className="bg-green-500 w-1/4  rounded mt-4">ADD</button> */}
                </div>

            </div>

        </div>

    );
};

export default PostReceiver;
